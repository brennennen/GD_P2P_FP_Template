...
func spawn_player(peer_id) -> Player:
	Log.info("spawn_player: %d" % [peer_id])
	...
	if is_instance_valid(spawn_points) and spawn_points.get_children().size() > 0:
		...
		Log.info("%s:spawn_player: player: %s pos: %v, rot.y: %d" % [name, player.name, spawn_point.global_position, spawn_point.rotation_degrees.y])
		spawn_point_global_position = spawn_point.global_position
		spawn_point_rotation = spawn_point.rotation
	else:
		Log.error("NO VALID SPAWN POINT! SPAWNING AT (0,0,0)")
...
func handle_player_death(player: Player):
	Log.info("handle_player_death: %s" % [ player.name ])
...
func respawn_player(player: Player, spectator: bool = false):
	if is_instance_valid(spawn_points) and spawn_points.get_children().size() > 0:
		...
		Log.info("respawn_player: %s, pos: %v, spectate: %s" % [ player.name, spawn_point.global_position, str(spectator) ])
		...
		Log.error("%s:respawn_player: NO VALID SPAWN POINTS!" % [name])
	...
	if allow_player_join_spawn_mid_match:
		for peer in GameInstance.networking.peers:
			if !GameInstance.networking.peers[peer].player:
				Log.info("Found player not spawned, spawning them: %s" % [str(peer)])
				spawn_player(peer)
...
func last_man_standing_1hz():
	...
	if players_alive == 1:
		...
		Log.info("TODO! game over! player still alive wins!")
		handle_game_over()
	if players_alive == 0:
		# gotta handle this
		Log.info("TODO! game over! figure out which player died last?")
		handle_game_over()
	Log.info("last_man_standing_1hz: players_alive: %d" % [players_alive])
