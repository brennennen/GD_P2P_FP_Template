...
func host_game() -> Error:
	Log.info("Creating direct connect server on: %d, max_connections: %d" % [port, max_connections])
	var error = enet_peer.create_server(port, max_connections)
	if error:
		Log.error("direct connect create_game error: %d" % error)
		return error
...
func join_game(address: String = "127.0.0.1", join_port: int = 7999) -> Error:
	...
	Log.info("Creating direct connect join client with address: %s, port: %d" % [address, join_port])
	...
	if error:
		Log.error("join_game error: (%d) %s" % [error, error_string(error)])
		return error
	multiplayer.multiplayer_peer = enet_peer
	return Error.OK
...
func transfer_lobby_data(lobby_data: Dictionary):
	Log.info("peer %d got lobby_data: %s" % [ multiplayer.get_unique_id(), JSON.stringify(lobby_data)])
